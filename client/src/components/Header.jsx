...
export default function Header() {
  ...
  return (
    ...
    transition={{ delay:0.5,duration:0.2}}
      ...
      >Generate Images
        <img className='inline w-5 ml-2' src={assets.star_group} alt="" /> 
      </motion.button>
      ...
      className='flex justify-center items-center flex-wrap gap-3 mt-16' >
        {Array(6).fill('').map((item, index) => (
          <motion.img 
          whileHover={{scale:1.1, duration:0.1}}
          key={index} className='inline w-10 sm:w-16 mt-10 mx-1' src={index % 2 === 0 ? assets.sample_img_2 : assets.sample_img_1} alt="" key={index} width={70}/>
        ))}
      </motion.div>
      <motion.p 
      initial={{opacity:0}}
